...
Vec3 viewport_get_point_from_pixel(Vec2 pixel) {
    const Vec3 viewport_u = {VIEWPORT_W, 0, 0};
    const Vec3 viewport_v = {0, -VIEWPORT_H, 0};
    ...
    const Vec3 pixel_upper_left = vec3_add(
        viewport_upper_left,
        vec3_div_n(vec3_add(pixel_delta_u, pixel_delta_v), 2)
    ); // pixel_upper_left = viewport_upper_left + (pixel_delta_u + pixel_delta_v) / 2

    return vec3_add(
        vec3_add(
            pixel_upper_left,
            vec3_mult_n(pixel_delta_u, pixel.x)
        ),
        vec3_mult_n(pixel_delta_v, pixel.y)
