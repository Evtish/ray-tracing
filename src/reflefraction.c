...
Vec3 get_specular_reflection_dir(const Vec3 dir, const Vec3 normal) {
    return vec3_sub(dir, vec3_mult_n(normal, 2 * vec3_dot(normal, dir))); // reflected_dir = dir - 2 * dot(normal, dir) * normal
}
...
Vec3 get_refraction_dir(const Vec3 dir, const Vec3 normal, const double cos_theta, const double rri) {
    ...
    // R' = R'⟂ + R'∥
    Vec3 refracted_dir_perp = vec3_mult_n(vec3_add(dir, vec3_mult_n(normal, cos_theta)), rri); // R'⟂ = η * (R + n * cosθ)
    Vec3 refracted_dir_paral = vec3_mult_n(normal, -sqrt(fabs(1 - vec3_squared_len(refracted_dir_perp)))); // R'∥ = n * (-√(1 - |R'⟂|²))
...
Vec3 reflefraction_get_dir(const Vec3 dir, const HitData *const hit_data) {
    ...
    switch (material.type) {
        default:
        case MAT_MATTE: return get_diffuse_reflection_dir(normal); break;
        case MAT_MIRROR: return get_specular_reflection_dir(dir, normal); break;
        case MAT_CONDUCTOR: return vec3_normalize(vec3_add(get_specular_reflection_dir(dir, normal), get_fuzzy_offset(material.fuzz))); break;
        case MAT_DIELECTRIC:
